  ...
  return (
    <div className="min-w-[250px] w-full rounded overflow-hidden shadow-lg">
      ...
      <div className="px-6 pt-4 pb-4">
        <Link className="px-2 py-1 font-semibold md:ml-0 text-white rounded-lg bg-slate-600 hover:bg-slate-900 ease-in-out duration-300  inline-block inline-flex items-center" to={`/services/${services?.id}`} > Checkout{" "} <span className="ml-2"> <BsFillCartCheckFill /> </span>{" "} </Link>
      </div>
    </div>
  );
};
